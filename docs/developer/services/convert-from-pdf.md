...
### Error Response Format (Readiness Check)

`/readyz` returns 503 with per-dependency results when a dependency is down (`/healthz`
liveness never returns this — it is a static `200 {"status":"ok"}`):
