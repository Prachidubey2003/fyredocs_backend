...
type VerifierConfig struct {
	AllowedAlgs []string
	HMACSecret  []byte
	// PreviousHMACSecret accepts tokens signed with the previous HS256 key
	// during a zero-downtime rotation window. New tokens are always signed
	// with HMACSecret; PreviousHMACSecret is verify-only. See SECRETS.md §3.
...
type Verifier struct {
	...
	hmacSecret         []byte
	previousHMACSecret []byte // optional; populated during JWT secret rotation
	rsaPublicKey       *rsa.PublicKey
	issuer             string
	audience           string
	clockSkew          time.Duration
...
func NewVerifier(config VerifierConfig) (*Verifier, error) {
	...
	return &Verifier{
		...
		previousHMACSecret: config.PreviousHMACSecret,
...
func (v *Verifier) Verify(ctx context.Context, tokenString string) (*Claims, error) {
	...
	// Dual-key fallback for HS256 zero-downtime rotation: if signature
	// verification failed and a previous secret is configured, retry once
	// with the previous secret. New tokens are always signed with the
	// current secret (see SECRETS.md §3).
...
// keyFuncPrevious is the dual-key counterpart used only when the primary
// signature check fails AND a previous HS256 secret is configured (i.e.,
// during a rotation window). It accepts HS256 only — RSA tokens never
// rotate this way; rotate RSA by deploying a new public key.
